...
node --test --experimental-test-coverage --test-coverage-include='js/logic.js' --test-coverage-include='js/registro.js' --test-coverage-include='js/carteira.js' --test-coverage-lines=80 tests/*.test.mjs > /dev/null 2>&1 || { echo "TESTES/COBERTURA FALHARAM - abortado"; exit 1; }
